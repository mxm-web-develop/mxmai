/**
 * Partner 短信登录迁移
 * pnpm --filter @mxmai/mxmdata run migrate:partner-sms-auth
 */
...
async function main() {
  ...
  const sql = readFileSync(
    join(MXMDATA_ROOT, 'src/database/migrations/add_partner_sms_auth.sql'),
    'utf8'
  );
  ...
  try {
    ...
    console.log('✅ partner_sms_auth 迁移完成');
  } catch (e: unknown) {
