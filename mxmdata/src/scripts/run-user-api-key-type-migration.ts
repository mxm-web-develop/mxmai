/**
 * 执行 user_api_keys.key_type 迁移
 * 运行: pnpm --filter @mxmai/mxmdata run migrate:user-api-key-type
 */
...
async function main() {
  ...
  const sqlPath = join(MXMDATA_ROOT, 'src/database/migrations/add_user_api_key_type.sql');
  ...
  try {
    ...
    console.log('✅ user_api_keys.key_type 迁移完成');
