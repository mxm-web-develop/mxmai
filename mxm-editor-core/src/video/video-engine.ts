...
export class VideoEngine {
  ...
  private renderSubtitleToCanvasCtx(
    ...
    const fontFamily = style?.fontFamily || "Inter, PingFang SC, Microsoft YaHei, sans-serif";
    const color = style?.color || "#ffffff";
    const backgroundColor = style?.backgroundColor || "rgba(0, 0, 0, 0.7)";
    const position = style?.position || "bottom";

    // 勿对已含逗号/引号的 font-family 再套一层引号，否则会回退默认 10px
    const familyPart =
      fontFamily.includes(",") ||
      fontFamily.trim().startsWith('"') ||
      fontFamily.trim().startsWith("'")
        ? fontFamily.trim()
        : /\s/.test(fontFamily.trim())
          ? `"${fontFamily.trim()}"`
          : fontFamily.trim();
    ctx.font = `bold ${fontSize}px ${familyPart}`;
