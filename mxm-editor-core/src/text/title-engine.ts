...
/** 组装 canvas `font` 用的 font-family，避免对 CSS 字体列表二次加引号 */
...
export class TitleEngine {
  ...
  private applyTextStyle(
    ...
    // fontFamily 可能是 CSS 列表（如 `"PingFang SC", Inter, sans-serif`）。
    // 外层再套引号会使整段 `ctx.font` 非法，浏览器回退默认 10px，overlay 几乎看不见。
    ctx.font = `${style.fontStyle} ${fontWeight} ${style.fontSize}px ${formatCanvasFontFamily(style.fontFamily)}`;
    ctx.fillStyle = style.color;
