...
export interface NanoBananaProParams extends GenerateParams {
...
export interface NanoBananaProResult extends GenerateResult {
  image_urls: string[];
...
async function generateImpl(
  ...
  if (params.image) {
    ...
    console.log(`[models/deerapi/graph/nano-banana-pro] 使用单张图片参数 (image): ${isBase64 ? 'Base64数据' : 'URL'}`);
...
const definition: ModelDefinition<NanoBananaProParams, NanoBananaProResult> = {
