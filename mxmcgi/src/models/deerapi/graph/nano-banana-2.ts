...
export interface NanoBanana2Params extends GenerateParams {
...
export interface NanoBanana2Result extends GenerateResult {
  image_urls: string[];
  progress?: AsyncIterable<any>;
}

const modelKey = 'nano-banana-2';
...
async function generateImpl(
  ...
  if (params.image) {
    ...
    console.log(
      `[models/deerapi/graph/nano-banana-2] 使用单张图片参数 (image): ${isBase64 ? 'Base64数据' : 'URL'}`
    );
...
const definition: ModelDefinition<NanoBanana2Params, NanoBanana2Result> = {
