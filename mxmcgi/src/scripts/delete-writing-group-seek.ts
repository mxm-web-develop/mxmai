/**
 * 删除已下线的 writing/group/seek（角度探索）。
 * 勿再通过 bundle upsert 复活；方案类请用 writing/group/deck（演示文稿）。
 *
 *   pnpm exec tsx src/scripts/delete-writing-group-seek.ts
 *   pnpm exec tsx src/scripts/delete-writing-group-seek.ts --dry-run
...
async function main() {
  ...
  const { data: routes, error: routeErr } = await client
    ...
    .select('id, task_key, sub_type, model')
    .eq('task_key', 'group')
    .eq('sub_type', 'seek');
  ...
  const { error: e3, count: c3 } = await client
    ...
    .delete({ count: 'exact' })
    .eq('business_type', 'writing-group-seek');
  if (e3) {
    console.warn('business_pricing 删除跳过:', e3.message);
  }

  console.log(
    `\n✅ 已删除 writing/group/seek：prompt=${c1 ?? 'n/a'} route=${c2 ?? 'n/a'} pricing=${c3 ?? 'n/a'}`
  );
}
