...
export async function generate(
  params: MinimaxSpeech02TurboParams,
  provider?: ProviderType
): Promise<MinimaxSpeech02TurboResult> {
  const modelName = 'minimax-speech-02-turbo';
  ...
  try {
    ...
    throw new Error(`MiniMax Speech-02-turbo 同步语音合成失败: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
export async function textToSpeech(
  text: string,
  options?: {
    voice_setting?: MinimaxSpeech02TurboParams['voice_setting'];
    audio_setting?: MinimaxSpeech02TurboParams['audio_setting'];
    pronunciation_dict?: MinimaxSpeech02TurboParams['pronunciation_dict'];
    timbre_weights?: MinimaxSpeech02TurboParams['timbre_weights'];
    stream?: boolean;
    stream_options?: MinimaxSpeech02TurboParams['stream_options'];
    language_boost?: string;
    output_format?: 'url' | 'hex';
    voice_modify?: MinimaxSpeech02TurboParams['voice_modify'];
    provider?: ProviderType;
  }
): Promise<MinimaxSpeech02TurboResult> {
  return generate({
