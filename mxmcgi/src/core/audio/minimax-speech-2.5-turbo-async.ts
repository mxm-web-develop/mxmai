...
export interface MinimaxSpeech25TurboAsyncParams extends GenerateParams {
  voice_setting?: {
...
export interface MinimaxSpeech25TurboAsyncResult extends GenerateResult {
...
export async function generate(
  params: MinimaxSpeech25TurboAsyncParams,
  provider?: ProviderType
): Promise<MinimaxSpeech25TurboAsyncResult> {
  const modelName = 'minimax-speech-2.5-turbo-async';
  ...
  try {
    ...
    throw new Error(`MiniMax Speech-2.5-turbo 异步语音合成失败: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
export async function textToSpeech(
  text: string,
  options?: {
    voice_setting?: MinimaxSpeech25TurboAsyncParams['voice_setting'];
    audio_setting?: MinimaxSpeech25TurboAsyncParams['audio_setting'];
    pronunciation_dict?: MinimaxSpeech25TurboAsyncParams['pronunciation_dict'];
    language_boost?: string;
    voice_modify?: MinimaxSpeech25TurboAsyncParams['voice_modify'];
    provider?: ProviderType;
  }
): Promise<MinimaxSpeech25TurboAsyncResult> {
  return generate({
