...
export async function generate(
  params: MinimaxSpeech26HdParams,
  provider?: ProviderType
): Promise<MinimaxSpeech26HdResult> {
  const modelName = 'minimax-speech-2.6-hd';
  ...
  try {
    ...
    throw new Error(`MiniMax Speech-2.6-hd 同步语音合成失败: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
export async function textToSpeech(
  text: string,
  options?: {
    voice_setting?: MinimaxSpeech26HdParams['voice_setting'];
    audio_setting?: MinimaxSpeech26HdParams['audio_setting'];
    pronunciation_dict?: MinimaxSpeech26HdParams['pronunciation_dict'];
    timbre_weights?: MinimaxSpeech26HdParams['timbre_weights'];
    stream?: boolean;
    stream_options?: MinimaxSpeech26HdParams['stream_options'];
    language_boost?: string;
    output_format?: 'url' | 'hex';
    voice_modify?: MinimaxSpeech26HdParams['voice_modify'];
    provider?: ProviderType;
  }
): Promise<MinimaxSpeech26HdResult> {
  return generate({
