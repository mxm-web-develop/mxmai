...
export interface MinimaxSpeech25HdParams extends GenerateParams {
  voice_setting?: {
...
export interface MinimaxSpeech25HdResult extends GenerateResult {
...
export async function generate(
  params: MinimaxSpeech25HdParams,
  provider?: ProviderType
): Promise<MinimaxSpeech25HdResult> {
  const modelName = 'minimax-speech-2.5-hd';
  ...
  try {
    ...
    throw new Error(`MiniMax Speech-2.5-hd 同步语音合成失败: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
export async function textToSpeech(
  text: string,
  options?: {
    voice_setting?: MinimaxSpeech25HdParams['voice_setting'];
    audio_setting?: MinimaxSpeech25HdParams['audio_setting'];
    pronunciation_dict?: MinimaxSpeech25HdParams['pronunciation_dict'];
    timbre_weights?: MinimaxSpeech25HdParams['timbre_weights'];
    stream?: boolean;
    stream_options?: MinimaxSpeech25HdParams['stream_options'];
    language_boost?: string;
    output_format?: 'url' | 'hex';
    voice_modify?: MinimaxSpeech25HdParams['voice_modify'];
    provider?: ProviderType;
  }
): Promise<MinimaxSpeech25HdResult> {
  return generate({
