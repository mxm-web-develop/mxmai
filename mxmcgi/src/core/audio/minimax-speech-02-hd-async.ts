...
export async function generate(
  params: MinimaxSpeech02HdAsyncParams,
  provider?: ProviderType
): Promise<MinimaxSpeech02HdAsyncResult> {
  const modelName = 'minimax-speech-02-hd-async';
  ...
  try {
    ...
    throw new Error(`MiniMax Speech-02-hd 异步语音合成失败: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
export async function textToSpeech(
  text: string,
  options?: {
    voice_setting?: MinimaxSpeech02HdAsyncParams['voice_setting'];
    audio_setting?: MinimaxSpeech02HdAsyncParams['audio_setting'];
    pronunciation_dict?: MinimaxSpeech02HdAsyncParams['pronunciation_dict'];
    language_boost?: string;
    voice_modify?: MinimaxSpeech02HdAsyncParams['voice_modify'];
    provider?: ProviderType;
  }
): Promise<MinimaxSpeech02HdAsyncResult> {
  return generate({
