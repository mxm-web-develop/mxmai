...
export interface MinimaxSpeech25HdAsyncParams extends GenerateParams {
  voice_setting?: {
...
export interface MinimaxSpeech25HdAsyncResult extends GenerateResult {
...
export async function generate(
  params: MinimaxSpeech25HdAsyncParams,
  provider?: ProviderType
): Promise<MinimaxSpeech25HdAsyncResult> {
  const modelName = 'minimax-speech-2.5-hd-async';
  ...
  try {
    ...
    throw new Error(`MiniMax Speech-2.5-hd 异步语音合成失败: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
export async function textToSpeech(
  text: string,
  options?: {
    voice_setting?: MinimaxSpeech25HdAsyncParams['voice_setting'];
    audio_setting?: MinimaxSpeech25HdAsyncParams['audio_setting'];
    pronunciation_dict?: MinimaxSpeech25HdAsyncParams['pronunciation_dict'];
    language_boost?: string;
    voice_modify?: MinimaxSpeech25HdAsyncParams['voice_modify'];
    provider?: ProviderType;
  }
): Promise<MinimaxSpeech25HdAsyncResult> {
  return generate({
