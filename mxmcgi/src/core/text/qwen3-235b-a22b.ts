...
export interface Qwen3235BA22BParams extends GenerateParams {
...
export interface Qwen3235BA22BResult extends GenerateResult {
...
export async function generate(
  params: Qwen3235BA22BParams,
  provider?: ProviderType
): Promise<Qwen3235BA22BResult> {
  // 这里使用我们统一的逻辑模型名 `qwen3-235b`
  const modelName = 'qwen3-235b';
  ...
  try {
    ...
    throw new Error(`Qwen3 235B A22B 生成失败: ${error instanceof Error ? error.message : String(error)}`);
  }
}
