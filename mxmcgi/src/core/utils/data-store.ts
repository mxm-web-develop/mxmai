/**
 * 数据存储工具
 * 用于将生图生视频接口返回的图片或视频数据存入 MinIO
 */

import { RepositoryFactory } from '@mxmai/mxmdata';
import { UploadOptions } from '@mxmai/mxmdata';
 ...
 */
async function downloadOrConvertToBuffer(data: string): Promise<{ buffer: Buffer; contentType: string; ext: string }> {
  // 检查是否是Base64数据
  if (data.startsWith('data:')) {
    ...
    const contentType = response.headers.get('content-type') || detectContentTypeFromBuffer(buffer) || 'application/octet-stream';
    const ext = getExtensionFromContentType(contentType) || getExtensionFromUrl(data) || 'bin';
 ...
 * 替换路径模板中的变量
 */
function replacePathTemplate(template: string, variables: Record<string, string | number>): string {
  let path = template;
