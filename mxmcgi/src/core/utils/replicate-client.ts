...
export class ReplicateClient {
  ...
  constructor(config?: ReplicateConfig) {
    ...
    if (!apiKey) {
      throw new Error('REPLICATE_API_TOKEN 环境变量必须设置，或通过 config.apiKey 提供');
    }
  ...
  async textToImage(
    ...
    try {
      ...
      // 调用 Replicate API
      const output = await this.replicate.run(model, { input }) as string | string[];
      ...
      return {
        image_urls: imageUrls.filter((url): url is string => typeof url === 'string' && url.length > 0),
      };
    } catch (error) {
      console.error('❌ Replicate 请求失败:', error);
      throw new Error(`Replicate 文生图请求失败: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async editImage(
    ...
    console.log('📤 请求参数:', JSON.stringify({ ...request, image: request.image ? '[已提供]' : undefined }, null, 2));
    }
    ...
    try {
      ...
      if (process.env.DEBUG_REPLICATE) {
      console.log('📤 Replicate 输入:', JSON.stringify({ ...input, image: '[已提供]' }, null, 2));
      }
      ...
      console.error('❌ Replicate 请求失败:', error);
      throw new Error(`Replicate 图片编辑请求失败: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
