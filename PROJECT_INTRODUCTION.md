...
- **Agent / Smartflow**：工作流与智能体（mxmagent）
...
| 微服务     | mxmauth / mxmpay / mxmcgi / mxmagent / mxmnotify，各占独立端口 |
...
- 路由：`/account`、`/assets` → mxmauth；`/payment`、`/wallets` → mxmpay；`/generation`、`/cgi/*`、`/system`、`/knowledge`、`/characters` → mxmcgi；`/agents`、`/smartflows` → mxmagent；`/notifications`、`/tasks`、`/sse`、`/task-events` → mxmnotify
