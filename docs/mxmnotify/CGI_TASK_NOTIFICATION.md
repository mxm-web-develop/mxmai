...
`metadata.task_snapshot` 与 `data.task` 字段对齐（列表增量 patch 用），**不**在 WS 中推送 base64 媒体。

| 字段 | 说明 |
|------|------|
| `id` / `status` / `type` | 任务标识与状态 |
| `progress` | `{ status, progress?, error? }` |
| `metadata.label` | 列表标题 |
| `createdAt` / `updatedAt` | ISO 时间 |
| `hasResult` / `media_count` | 终态是否有结果（不含 URL） |

进行中会收到节流后的 `task_updated`（默认每 5s 或进度跨 10%）；`queued` / `processing` 各推一次。
