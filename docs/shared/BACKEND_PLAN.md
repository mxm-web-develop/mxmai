...
├── mxmpay/          # 支付业务模块
├── mxmauth/         # 用户管理业务模块
├── mxmcgi/          # AI 生成 + Task V2 + Smartflow + Agent
├── mxmnotify/       # 通知业务模块
└── gateway/         # API网关
```
...
### 2.4 Agent / Smartflow（mxmcgi 内模块）
...
**详细设计：** 参见 [MXMAGENT.md](./MXMAGENT.md)（指向 mxmcgi 实现）
...
2. **中优先级**：mxmcgi 内 Smartflow/Agent 相关表（若需独立库）
