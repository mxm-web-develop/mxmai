...
var NotificationService = class {
  ...
  async sendTaskCompletedNotification(task) {
    const taskTypeLabel = task.task_type === "graph" ? "\u56FE\u7247\u751F\u6210" : task.task_type === "outline" ? "\u5927\u7EB2\u751F\u6210" : "\u6587\u672C\u751F\u6210";
    ...
    try {
      ...
      for (let i = 0; i < notificationIds.length; i += BATCH_SIZE) {
        ...
        try {
          ...
          const { data: notifications, error: fetchError } = await supabase.from("notifications").select("id, user_id").in("id", batch);
          if (fetchError) {
...
// src/index.ts
(0, import_mxmdata5.loadMonorepoEnv)({ service: "mxmnotify" });
try {
...
var port = Number(process.env.MXMNOTIFY_PORT || process.env.PORT || 4005);
