...
mxmnotify 负责统一接收来自各业务模块（mxmpay、mxmcgi、mxmauth、mxmagent 等）的事件，按照模板生成通知，并通过 API / WebSocket / Push 推送给终端。核心功能：
...
type AsyncTaskStatusChangedEvent = {
  ...
  module_type: 'mxmpay' | 'mxmcgi' | 'mxmauth' | 'mxmagent' | string;
  task_id: string;
  user_id: string;
